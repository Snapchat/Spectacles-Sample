...
export class MatchTransform extends BaseScriptComponent {
    ...
    private initializeOffset() {
        ...
        print(`MatchTransform (${this.getSceneObject().name}): Object world pos: ${myWorldPos}, Finger pos: ${this.targetPosition}`)
    }
    ...
    private updateTransform() {
        ...
        if (this.updateCounter % 10 === 0) {
            ...
            print(`MatchTransform (${this.getSceneObject().name}): dist=${dist.toFixed(2)}cm, transition=${(this.offsetTransitionProgress*100).toFixed(0)}%`)
        }
